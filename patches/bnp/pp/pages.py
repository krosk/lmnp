...
class LoginPage(PartialHTMLPage):
    def login(self, username, password):
        # BNP replaced the image-based virtual keyboard with a JS button grid.
        # The page now embeds `var gridNumbers="XXXXXXXXXX"` (randomised each load)
        # where position i in the string is the digit shown at grid slot i.
        grid_numbers = Regexp(
            CleanText('//script[contains(text(), "gridNumbers")]'),
            r'var gridNumbers="(\d+)"',
        )(self.doc)
        # Map each password digit to its 1-indexed slot in the grid (zero-padded to 2 chars).
        grid_position = "".join("%02d" % (grid_numbers.index(d) + 1) for d in password)

        form = self.get_form(id="identForm")
        form["userGridPasswordCredential.username"] = username
        form["userGridPasswordCredential.gridPosition"] = grid_position

        # We must check redirections one by one after sending this form for errors or OTPs
        form.submit(allow_redirects=False)

    def get_error(self):
        return Regexp(CleanText('//form[@id="identForm"]//script//text()'), r"errorMessage = \[\"([^\"]+)\"\]")(
            self.doc
        )
