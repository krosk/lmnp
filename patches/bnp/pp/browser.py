...
class BNPParibasBrowser(LoginBrowser, StatesMixin):
    ...
    document_date_range = URL(r"/demat-wspl/rest/rechercheCriteresDemat", DocumentsPage)
    ...
    def iter_accounts(self):
        if self.accounts_list is None:
            ...
            # Life insurance (Assurances Vie / capitalisation) probe skipped:
            # clients.assurance-vie.fr times out and no life insurance accounts exist.
...
class DocumentsBNPParibasBrowser(BNPParibasBrowser):
    ...
    def iter_documents(self, subscription):
        ...
        # rechercheCriteresDemat accepts a date range and returns mapDocuments —
        # a superset of what listerDocuments returns, without any month cap.
        # Fetch year by year from 5 years ago to today.
        self.titulaire.go()
        end = datetime.now()
        start = end - relativedelta(years=5)
        for year_start in (start + relativedelta(years=i) for i in range(end.year - start.year + 1)):
            year_end = min(year_start + relativedelta(years=1) - relativedelta(days=1), end)
            self.document_date_range.go(
                json={
                    "dateDebut": year_start.strftime("%d/%m/%Y"),
                    "dateFin": year_end.strftime("%d/%m/%Y"),
                }
            )
            if self.page.has_error():
                continue
            for doc in self.page.iter_documents_date_range(
